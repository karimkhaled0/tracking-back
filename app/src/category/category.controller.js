...
import { validateCategoryInput } from "../../validation/category.js";
  ...
  try {
    if (req.user.isAdmin) {
      ...
      for (let index = 0; index < category.technicals.length; index++) {
        const technical = await User.findByIdAndUpdate(
          { _id: category.technicals[index] },
          { $unset: { category: category._id } },
          { new: true }
        );
