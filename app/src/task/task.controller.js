...
export const updateTask = async (req, res) => {
    ...
    try {
        ...
        if (!req.user.isAdmin || !req.user.isTeamLeader) {
            const updatedTask = await Task.findByIdAndUpdate({ _id: req.params.id }, { started: req.body.started, report: req.body.report, inReview: req.body.inReview, inProgress: req.body.inProgress, finished: req.body.finished }, { new: true })
                .lean()
                .exec()
...
export const deleteTask = async (req, res) => {
    try {
        if (req.user.isAdmin || req.user.isTeamLeader) {
            ...
            const category = await Category.findByIdAndUpdate({ _id: task.category }, { $pull: { tasks: task._id } }, { new: true });
